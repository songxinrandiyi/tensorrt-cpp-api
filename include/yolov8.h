...
struct YoloV8Config {
    ...
    std::vector<std::string> classNames = Config::classNames;
};
...
private:
    ...
    const std::vector<std::vector<float>> COLOR_LIST = Config::COLOR_LIST;

    const std::vector<std::vector<unsigned int>> KPS_COLORS = Config::KPS_COLORS;

    const std::vector<std::vector<unsigned int>> SKELETON = Config::SKELETON;

    const std::vector<std::vector<unsigned int>> LIMB_COLORS = Config::LIMB_COLORS;
};